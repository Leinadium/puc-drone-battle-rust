...
pub fn get(f: &Field, c: &Coord) -> Position {
    ...
    else { f.map.get(c).unwrap_or(&Position::UNKNOWN).clone() }
}
...
fn has_something_to_collect(
    ...
    for (something_coord, time) in hm.iter() {
        ...
        match &current_path {
            ...
            Some(cp) => {
                if f.config.spawn_timer < f.config.normal_timer.mul(cp.size as u32) + *time {     // it will spawn in time
                    match &best_path {
...
pub fn has_wall_front(f: &Field, c: &Coord, dir: &PlayerDirection, q: i16) -> bool {
